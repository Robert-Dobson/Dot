...
class OtherCog(commands.Cog):
    ...
    async def random(self, interaction, num: int):
        if num <= 0:
            await interaction.response.send_message("Must provide a number greater than 0")
            return

        await interaction.response.send_message(f"Random number is: {rand.randint(1, num)}")
